...
const InCinemasMovies = ({ limit = 4, allView = false }) => {

    const { inCinemasMves } = useContext(TMDBContext);

    if (!inCinemasMves.length) {
        return <p className="text-center">Loading Currently Playing Movies...</p>;
    }

    const displayMves = allView ? inCinemasMves : inCinemasMves.slice(0, limit);

    return (
        <div className="max-w-4xl mx-auto p-4">
            <h2 className="text-3xl font-bold text-center mb-6 text-white">
                {allView ? 'All Currently Playing Movies' : 'Currently Playing Movies'}
            </h2>
